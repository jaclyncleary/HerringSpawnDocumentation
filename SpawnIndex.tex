...
\begin{itemize}
\item Ashleen Benson for translating the \textbf{Microsoft Access} database to an \textbf{R} script, which we referenced when developing \texttt{SpawnIndex.R} (and for reviewing?)
\item Jake Schweigert for background and justification of the manual corrections (\autoref{subsecUpdate}; and for reviewing?)
\end{itemize}
Also, we need two reviewers; they should be mentioned here too (Ashleen and Jake come to mind).
