...
Generally, Pacific herring spawn on [hard?] substrate and algae from the intertidal to subtidal.
From above, herring spawns are typically bands running parallel to the shore.
Each distinct spatial or temporal spawn has a unique `spawn number.'
The spawn number is the finest scale that we calculate the spawn index.

The first task is to map out the spatial extent of the herring spawn.
Transects are set perpendicular to the shore.
Generally, transects are set $200\,\text{m}$ in from the ends of the spawn, and spaced $350\,\text{m}$ apart.
Most areas on the coast where herring spawn have permanent transects to allow survey crews to replicate transects from previous years.
When permanent transects are not available, survey crews create new transects based on pre-specified criteria.
We digitize these new transects, which are then available as permanent transects.

In this report, `length' usually refers to distances parallel to the shore, and `width' usually refers to distances perpendicular to the shore.
Thus, `Macrocystis length' and `vegetation length' refer to the distance that Macrocystis and vegetation extends parallel to the shore, respectively.
One exception is transect width, $TransectWidth = 2\,\text{m}$, which refers to the distance parallel to the shore because transects are set perpendicular to the shore.
...
For each mature plant, divers record plant height, number of egg layers, and number of fronds.
...
In addition, divers identify (at most) three dominant vegetation types that have spawn.
For each of these vegetation types, divers record the percentage of the quadrat covered by the vegetation, and number of egg layers.
