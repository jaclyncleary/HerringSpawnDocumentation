...
\newcommand{\trAuthsLong}{Matthew H.~Grinnell, and others...}  % JC, MT, ...
...
\pdfbookmark[1]{\contentsname}{toc}  % Add TOC to pdf bookmarks
...
First, the \textbf{R} script is open and transparent; users are welcome to view and download the script and example data (\autoref{secDown}).
Second, the database has been used for various purposes over several decades, and has incidental calculations that make it overly complex.
Third, the database is difficult to troubleshoot because most of the authors have retired.
...
As previously mentioned, the \textbf{R} script to calculate the Pacific herring spawn index, \texttt{Spawn.R} is publicly accessible on \textbf{GitHub} (URL?).
% Something about the script being 'well commented'.
...
\begin{itemize}
\item Jake Schweigert for background/reasoning re manual corrections
\end{itemize}
