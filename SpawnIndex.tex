...
Projected spawning biomass is used to create harvest decision tables.
...
Most areas have `permanent transects' which are plotted on maps to enable crews to place transects in the same location each year.
...
Preliminary (MT):
Dive surveys works off regularly spaced transects and sampling stations, while surface survey sampling intervals are subject to surveyor judgement.
If the spawning bed is large enough, surface surveyors usually work from the same transect charts as dive surveys.
Small spawns can still be mapped as they were historically, with the surveyor deciding how to divide the bed up into sections.
End of spawns are mapped to determine spawn length.
Regular rakes throughout the spawn bed determine vegetation type, number of egg layers and vegetation coverage.
...
For each transect, divers record the number of egg layers.
