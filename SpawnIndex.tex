...
This report is divided into sections: in each section we describe the spawn index calculations for one of the aforementioned spawn survey types.
...
However, we first quantify Pacific herring fecundity, which is critical to calculating the spawn index.
Finally, in the last section, we combine the three spawn indices to a total annual spawn index by SAR, which is the appropriate scale for Pacific herring stock assessments in BC.
...
To correct errors in the surface spawn database, we make several manual updates to $EggLyrsTotT$ for the following records:
\begin{enumerate}
...
Essentially, these updates correct missing or inaccurate egg layer information (J.~Schweigert, personal communication, 21 February 2017).
For example, some updates use spawn intensity values (Table \ref{tabIntensity}) to determine egg layers because of missing or inaccurate egg layer information (i.e., \#~\ref{up1979}, \ref{up1981}, \ref{up1982a}).
This is possible because intensity values were still reported after 1978 in some cases.
...
For stock assessment purposes, the annual total spawn index is aggregated to the scale of the SAR.
As previously mentioned, the spawn index must be scaled by the spawn survey scaling parameter, $q$ \citeyearpar[CSAS][]{CSAS2015b} before it is considered to be an estimate of spawning stock biomass.
