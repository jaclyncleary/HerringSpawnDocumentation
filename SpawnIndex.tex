...
Second, we provide a brief overview of the three types of spawn survey observations: surface observations, underwater observations of spawn on Macrocystis ($Macrocystis$ spp.), and underwater observations of spawn on understory.
Third, we describe the calculations for each of the three spawn survey types: surface, Macrocystis, and understory.
Fourth, we combine the three spawn indices, and aggregate by year and stock assessment region to match the temporal and spatial scale of science advice and fishery management for Pacific herring in BC.
Fifth, we describe some sources of uncertainty in the spawn index.
Finally, the provide our script and example data so that users can calculate the Pacific herring spawn index themselves.
...
%Therefore, we divide $fecundity$ by $10^{3}$ ($= 10^{5}$) to calculate an index of spawning biomass (i.e., the spawn index) in tonnes.
