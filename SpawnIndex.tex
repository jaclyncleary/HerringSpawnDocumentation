...
For example, there are (at least) two ways for \autoref{eqEggDensUnderV}.
...
\item For surface spawn, `length' refers to distances parallel to the shore, and `width' refers to distances perpendicular to the shore.
...
\item For Macrocystis spawn, `length' generally refers to distances parallel to the shore, and `width' generally refers to distances perpendicular to the shore.
\item The only exception is transect width, $WidthT = 2\,\text{m}$, which refers to the distance parallel to the shore.
...
\item Quadrat size for understory spawn surveys is usually $0.5\,\text{m}^{2}$; what is spacing between quadrats?
\item Other quadrat sizes have been used for research in the past, but not for spawn surveys.
\item For understory spawn, `length' refers to distances parallel to the shore, and `width' refers to distances perpendicular to the shore.
...
\end{equation}
where $Width$ is the transect length (i.e., the spawn width) in metres, and $EggDensWtQ$ is in $\text{Eggs} \cdot 10^{3} \cdot \text{m}^{-1}$.
...
Second, observation error may affect input data such as the number of egg layers, and model strutural complexity may affect estimated prediction model parameters (e.g., \autoref{eqEggDensSurf}).
...
Investigating and quantifying uncertainty may increase trust in the spawn index, and identify weaknesses to target with future analyses.
In addition, acknowledging uncertainty in the calculations will reduce another source of uncertainty, inadequate communication among scientists, managers, and stakeholders, which can lead to misapplication of scientific advice \citep{LinkEtal2012}.
...
Investigating these trade-offs is critical to acquiring sufficient spawn data, and understanding precision loss, if spawn survey effort is reduced in the future.
...
\end{enumerate}
Quantifying and reporting spawn index uncertainty will increase transparency, and enable users to assess potential impacts to, for example, Pacific herring stock assessments in a management strategy evaluation approach.
