...
\newcommand{\trAuthBDetails}{E-mail: \texttt{Other.Authors@dfo-mpo.gc.ca} $|$ telephone: (250)~765.XXXX}
...
Our goal is to collect and simplify the details necessary to understanding how the spawn index is calculated.

The motivation to document the spawn index calculations came when we translated the spawn index calculations from a \textbf{Microsoft Access} database to an \textbf{R} script \citeyearpar[RCT][]{R-3.3.2}, \texttt{Spawn.R}.
...
And finally, the database is difficult to troubleshoot because most of its authors have retired.
...
However, we first quantify Pacific herring fecundity, which is critical to calculating the spawn index (\S~\ref{secFecund}).
Finally, we combine the three spawn indices to get the total spawn index (\S~\ref{secTotal}).

Note that sections in this report correspond somewhat to functions in the \textbf{R} script, \texttt{Spawn.R}.
...
Essentially, the \textbf{R} script imports data from the Pacific herring spawn database (Ref?), and does the calculations described in this report.
...
For example, in some cases we update $EggLyrsTotT$ based on `intensity' (Table \ref{tabIntensity}), which was sometimes recorded after being officially discontinued in 1978.
...
\item Update $EggLyrsTotT$ to 2.33 for the 41 records in the year 1984, statistical area 24, and with intensity 0; and \label{up1984}
\item Update $EggLyrsTotT$ to 2.98 for the 14 records in the year 1982, statistical area 27, and with $EggLyrsTotT = 0$. \label{up1982b}
\end{enumerate}
...
%\section{AN EXAMPLE}
%We could include a simple example where we work through the calculations step-by-step for each spawn survey type.
%It would take some time/work to make this useful: show the input data, work through the calculations, results, etc.
%
