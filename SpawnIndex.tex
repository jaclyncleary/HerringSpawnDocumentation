...
The spawn index calculations have been updated over the years as more data become available; we restrict this report to describing the current methods.%
...
I suspect this isn't necessary, and would open up a lot of possibly unnecessary/irrelevant history.. there are a lot of `old' ways to calculate the spawn index.}
...
We have divided this report into sections to divide topics.
...
\begin{itemize}
\item Brief description of surface spawn surveys.%
\footnote{I think we should provide a brief/general description for each survey type because I'm not sure how to reference the `Herring spawn survey manual.' Maybe the full details, including for example, the rational for transect and quadrat size and interval, should be in a separate Technical Report so that we can reference it?}
\item Generally, `length' refers to distances parallel to the shore, and `width' refers to distances perpendicular to the shore.
\item (MT) Surface surveys can follow the pre-determined transects of a dive survey chart, or can make their own spawn area... we can discuss and clarify further.
\end{itemize}
...
Also, \citet{HayKronlund1987} says that the change from 5 to 9 categories happened in 1969, which seems to be incorrect based on intensity values in the database.}
...
Macrocystis plants are categorized as either `mature' or `immature' based on stipe height.
Mature plants have stipes $\geq1\,\text{m}$ high, and are the only plants used for Macrocystis spawn index calculations.
\begin{itemize}
\item Brief description of Macrocystis spawn surveys.
\item Transect width is $2\,\text{m}$.
\item Generally, `length' refers to distances parallel to the shore, and `width' refers to distances perpendicular to the shore.
\item Thus, `Macrocystis length' is the distance that Macrocystis extends parallel to the shore?
\end{itemize}
...
\begin{itemize}
\item Brief description of Understory spawn surveys.
\item Quadrat size is usually $0.5\,\text{m}^{2}$; what is spacing between quadrats?
\item Generally, `length' refers to distances parallel to the shore, and `width' refers to distances perpendicular to the shore.
\item Thus, `vegetation length' is the distance that vegetation extends parallel to the shore?
\end{itemize}
...
%Note that \autoref{eqEggDensUnderV} replaces an earlier equation (there were a few 'earlier' equations..) for vegetation egg density which is no longer in use:
%\begin{multline}
...
At the spawn level, the sum of transect widths $Width$ is $WidthTotS$, the mean of $Width$ is $WidthMeanS$, and the vegetation length is $LengthVegS$, all in metres.
If $LengthVeg$ is inadvertently not recorded, we set $LengthVeg$ to the transect length $Length$.
...
\begin{equation}
UnderSI = \frac{EggDensWtS \times LengthVegS \times WidthMeanS} {1 \cdot 10^{5}}
...
Like all ecosystem models, spawn index calculations may be affected by various potential sources of uncertainty including natural variability, observation error (e.g., bias, imprecision), and model structural complexity \citep{LinkEtal2012}.
For example, natural variability may affect estimated Pacific herring fecundity, and the sex ratio of spawning herring (\autoref{eqFecundityConv}).
In addition, observation error may affect input data such as the number of egg layers, and model strutural complexity may affect estimated prediction model parameters (e.g., \autoref{eqEggDensSurf}).
However, uncertainty in the spawn index has typically been omitted, which may create the wrong impression that the spawn index is observed data as opposed to derived data.

Investigating and quantifying uncertainty could improve our trust in the spawn index, and identify weaknesses to target for further analysis.
In addition, acknowledging uncertainty will reduce another source of uncertainty, inadequate communication among scientists, managers, and stakeholders, which can lead to misapplication of scientific advice \citep{LinkEtal2012}.
Analyses to quantify spawn index uncertainty include:
\begin{enumerate}
\item Collect biological samples to quantify egg production rates and sex ratios, which may be influenced by time-varying biological processes such as the observed non-stationarity of weight-at-age;
...
Quantifying uncertainty may also identify options to reduce spawn survey costs. 
Two reasons to investigate these options are: there is a trade-off between spawn index precision versus survey cost; and there is a trade-off between high precision in some areas versus low or no information in other areas.
Strategies to reduce spawn survey costs include:
\begin{enumerate}
\item Conduct underwater surveys for major spawns in core areas, and surface surveys for all other areas;
\item Quantify survey precision with respect to transect interval and width, as well as quadrat interval and size \citep[see][]{Schweigert1993b}; and
\item Conduct periodic versus annual surveys.
\end{enumerate}
Quantifying and reporting spawn index uncertainty will increase transparency, and enable users to assess potential impacts to, for example, Pacific herring stock assessments in a management strategy evaluation process.
