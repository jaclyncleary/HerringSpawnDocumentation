...
Next, we describe the calculations for each of the three aforementioned spawn survey types: surface (\autoref{secSurf}), Macrocystis (\autoref{secMacro}), and understory (\autoref{secUnder}).
...
Then, we combine the three spawn indices to get the total spawn index (\autoref{secTotal}).
Next, we describe how users can download and run the \textbf{R} script to calculate the spawn index (\autoref{secDown}).
Finally, we suggest some analyses to help quantify the assumptions and uncertainties in spawn index calculations (\autoref{secWork}).
...
\footnote{I think we should provide a brief/general description for each survey type because I'm not sure how to reference the `Herring spawn survey manual.'}
...
\section{FUTURE WORK}\label{secWork}
The spawn index calculations rely on assumptions such as Pacific herring fecundity and the sex ratio of spawning herring.
In addition, calculations ignore uncertainty in input data such as the number of egg layers, or prediction models (e.g., \autoref{eqEggDensSurf}).
Investigating these sources of uncertainty could improve our trust in the spawn index, identify weaknesses to target for further analysis, or identify strategies to reduce spawn survey costs.
Suggestions for future work include:
\begin{itemize}
\item Test fecundity assumptions by collecting biological samples to estimate egg production rates and sex ratios. For example, the observed non-stationarity of biological processes (e.g., weight-at-age) could affect egg production rates;
\item Spawn index sensitivity analysis (either global by specifying a distibution to each variable, or one variable at a time by increasing/decreasing by a certain proportion);
\item Investigate trade-off between spawn index precision versus survey cost/effort, as well as trade-off between high spawn index precision in some areas versus low or no information in other areas:
\begin{itemize}
\item Transect interval and width, as well as quadrat interval and size;
\item Underwater surveys for major spawns in core areas, and surface surveys for all other areas;
\item Periodic versus annual surveys;
\end{itemize}
\item Quantify uncertainty due to variability in estimated prediction model parameters (e.g., \autoref{eqEggDensSurf}); and
\item Bootstrap quadrats within transects, and transects within spawns \citep[see][]{Schweigert1993b}.
\end{itemize}
Finally, quantifying and reporting these sources of uncertainty will make the spawn index more transparent, and enable us to assess potential impacts on Pacific herring stock assessments.
